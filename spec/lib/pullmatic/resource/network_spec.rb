...
EOF
      ...
      describe ".execute" do
        it "should generate resolv" do
          ret = described_class.execute
          expect(ret.keys).to include(:resolv)
          expect(ret[:resolv][0]).to eq "nameserver 10.150.0.2"
          expect(ret[:resolv][1]).to eq "nameserver 10.150.132.5"
        end
        before do
          allow(Specinfra::Command::Linux::Base::Inventory).to receive(:get_resolv).and_return(
<<EOF
cat << EOF2
; generated by /sbin/dhclient-script
nameserver 10.150.0.2
nameserver 10.150.132.5
EOF2
EOF
)       end
      end
